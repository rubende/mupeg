...
INPUT_PATH = "/inputs_N50/"                     # Input data from TUM-GAID dataset. Into /inputs_N155/ we have 3 folders, one for video type. Into this folders, we stored one tfrecord file per video.
OUTPUT_PATH = "/outputs/"                       # Output path, where we store the resulting model.
LOG_DIR = "./logs/casia_50/"                    # Tensorboard log path
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        filenames0, val_filenames0 = \
            createDatasetRecordCasiaB.create_dataset_ft(INPUT_PATH, 0.1)

        # Create TFRecord to val
        val_images, val_labels = \
            createDatasetRecordCasiaB.create_tfrecord_ft(batch, num_class, val_filenames0,  mean=mean_dataset)

        # Create TFRecord to train
        images, labels = createDatasetRecordCasiaB.create_tfrecord_ft(batch, num_class, filenames0,  mean=mean_dataset)
        ...
        model = ModelCasiaB.Network()
        model.load_to_ft(model_50_path, num_class, dropout)
